...
<head>
    ...
    <title>Student Courses | SkillPro Institute</title>
...
<body>
    ...
    <header class="navbar">
        ...
        <nav>
            <ul class="nav-links">
                ...
                <li><a href="{{ route('student.course') }}" class="active">Courses</a></li>
        ...
        <script>
            function confirmLogout() {
                Swal.fire({
                    ...
                    iconColor: '#008851', background: '#f9f9f9', color: '#333',
                    showCancelButton: true, confirmButtonColor: '#008851', cancelButtonColor: '#6c757d',
                    confirmButtonText: 'Yes, logout', cancelButtonText: 'No, stay here'
  ...
  <section class="filter-section">
        ...
        <form action="{{ route('student.course') }}" method="GET" class="filter-form">
            <div class="search-bar-container"><input type="text" name="search" placeholder="Search courses by name..." value="{{ request('search') }}"></div>
            <div class="filter-controls">
                <select name="category"><option value="">All Categories</option>@foreach($categories as $category)<option value="{{ $category->catid }}" {{ request('category') == $category->catid ? 'selected' : '' }}>{{ $category->category }}</option>@endforeach</select>
                <select name="location"><option value="">All Locations</option>@foreach($locations as $location)<option value="{{ $location->locid }}" {{ request('location') == $location->locid ? 'selected' : '' }}>{{ $location->location }}</option>@endforeach</select>
                ...
                <select name="instructor"><option value="">All Instructors</option>@foreach($instructors as $instructor)<option value="{{ $instructor->id }}" {{ request('instructor') == $instructor->id ? 'selected' : '' }}>{{ $instructor->fullname }}</option>@endforeach</select>
                <a href="{{ route('student.course') }}" class="reset-btn">Reset</a>
            </div>
        </form>
    </section>
    ...
    <main class="courses-grid">
        @if($courses->isEmpty())
            <div class="no-courses"><p>No courses found for the selected filters. Try resetting the filters.</p></div>
        @else
    ...
    {{-- Registration Popup --}}
    <div id="registrationPopup" class="popup-modal">
        <div class="popup-content">
            <button id="closeRegPopup" class="close-btn">&times;</button>
            <form id="registrationForm" class="registration-form" action="{{ route('student.storeRegistration') }}" method="POST" novalidate>
                @csrf
                <h2>Register for Course</h2>
                <input type="hidden" id="formCourseId" name="course_id[]">
                <div class="form-group">
                    <label for="formCourseName">Course Name</label>
                    <input type="text" id="formCourseName" readonly>
                </div>
                <div class="form-actions">
                    <button type="button" id="cancelRegBtn" class="form-btn cancel-btn">Cancel</button>
                    <button type="submit" class="form-btn confirm-btn">Confirm</button>
                </div>
            </form>
        </div>
    </div>

    <script src="{{ asset('js/studentcourse.js') }}"></script>
</body>
</html>
